...
import lib_zstack
import lib_zstack_constants as lib_const
...
class Session(pgraph.Graph):
    ...
    def __init__(
        ...
        if threshold is None:
            raise exception.BoofuzzError("Threshold must not be none! Please input a positive number.")
        ...
        self._crash_filename = "taintbfuzz-crash-bin-{0}".format(self._run_id)
        ...
        # Coverage-guided related
        # Set up bitmap to record trace
        lib_zstack.setup_bitmap(self)
        # Record current mutant of BooFuzz before selecting seed from pending favored list.
        # Also used to resume state when no more pending favored seeds.
        self.last_mutant = []
        # Threshold used to switch to mutation with dependency inference
    ...
    def _callback_current_node(self, node, edge):
        ...
        if edge.callback:
            self._fuzz_data_logger.open_test_step("Callback function %s" % edge.callback.__name__)
    ...
    def transmit_fuzz(self, sock, node, edge, callback_data):
        ...
        try:  # send
            start_time = helpers.get_milli_time()
            self.targets[0].send(data)
            self.last_send = data
        except exception.BoofuzzTargetConnectionReset:
        ...
        try:  # recv
            if self._receive_data_after_fuzz:
                self.last_recv = self.targets[0].recv()
                end_time = helpers.get_milli_time()
                self.coverage.current_exec_us = end_time - start_time
        except exception.BoofuzzTargetConnectionReset:
            if self._check_data_received_each_request:
                self._fuzz_data_logger.log_fail(constants.ERR_CONN_RESET)
            else:
    ...
    def _iterate_single_node(self, path):
        ...
        mutated = self.fuzz_node.mutate()
        dependency_file = open(lib_const.dependency_file, 'r')
        dependency = json.load(dependency_file)
        self.infer_flag = False
        coverage_stat = open(lib_const.coverage_stat_dir + "coverage_stat_" + self.coverage.init_timestamp + ".txt", "a")
        coverage_stat.write(("Mutation Threshold: %d" % self.threshold).decode())
        coverage_stat.close()
        while mutated:
            self.total_mutant_index += 1
            yield (path,)
            if len(self.last_mutant) == 0:
                self.last_mutant = copy.copy(self.fuzz_node.render_list)
            ...
            current_favored = self.coverage.current_favored
            if self.coverage.pending_favored and current_favored is not None:
                self._fuzz_data_logger.open_test_step("Current favored: %s (%s)" % (current_favored.mutant_name, repr(current_favored.value)))

                if self.coverage.last_coverage['coverage'] == self.coverage.bitmap_state['edge'] and \
                        self.total_mutant_index - self.coverage.last_coverage['test_case'] > self.threshold:
                    self.infer_flag = True
                    depend_fields = lib_zstack.check_field_dep(self, self._fuzz_data_logger, dependency)
                    if current_favored.last_check_depfields is None and len(depend_fields) != 0:
                        current_favored.last_check_depfields = depend_fields
                    elif current_favored.last_check_depfields is not None and len(current_favored.last_check_depfields) > 0:
                        depend_fields = copy.copy(current_favored.last_check_depfields)
                    else:
                        self.infer_flag = False

                    if len(depend_fields) != 0:
                        mutated = current_favored.mutate(self._fuzz_data_logger, self.fuzz_node, self.infer_flag, depend_fields)
                        self._fuzz_data_logger.log_info("Fuzzing dependent field: %s" % self.fuzz_node.mutant._name)
                    else:
                        mutated = current_favored.mutate(self._fuzz_data_logger, self.fuzz_node)
                else:
                    mutated = current_favored.mutate(self._fuzz_data_logger, self.fuzz_node)

                if mutated:
                    self.fuzz_node._mutant_index += 1
                else:
                    current_favored.complete_mutate(self.fuzz_node)
                    if len(self.coverage.top_rated) == 1:
                        self.coverage.interest[0] = copy.copy(current_favored)
                        self.coverage.top_rated.pop(-1)
                        self.infer_flag = False
                        self.coverage.pending_favored = 0
                    elif len(self.coverage.top_rated) > 1:
                        for i, interest in enumerate(self.coverage.interest):
                            if current_favored.equal(interest):
                                self.coverage.interest[i] = copy.copy(current_favored)
                                break
                        self.coverage.top_rated.pop(-1)
                        self.infer_flag = False
                        lib_zstack.cull_favored(self.infer_flag, self.fuzz_node, self.coverage)
                        if len(self.coverage.top_rated) == 0:
                            self.coverage.pending_favored = 0
                        else:
                            self.coverage.current_favored = self.coverage.top_rated[-1]
                            lib_zstack.reset_favored(self.fuzz_node, self.coverage.current_favored)
                            self._fuzz_data_logger.open_test_step("New favored: %s (%s)" %
                                (self.coverage.current_favored.mutant_name, repr(self.coverage.current_favored.value)))

                    if self.coverage.pending_favored == 0:
                        self.infer_flag = False
                        self.infer_uncovered = []
                        self.curr_dep_func_index = 0
                        self.last_dep_path = None
                        self.last_path_hash = 0
                        self.fuzz_node.reset()
                        for node in self.last_mutant:
                            original = self.fuzz_node.names[node._name]
                            original._value = node._value
                            original._mutant_index = node._mutant_index
                        self.fuzz_node.render_list = []
                        self.last_mutant = []
                        self.coverage.current_favored = None
                        mutated = self.fuzz_node.mutate()
                    else:
                        mutated = self.coverage.current_favored.mutate(self._fuzz_data_logger, self.fuzz_node)
            else:
                # if self.coverage.last_coverage['coverage'] == self.coverage.bitmap_state['edge'] and \
                #         self.total_mutant_index - self.coverage.last_coverage['test_case'] > self.threshold:
                #     self.infer_flag = True
                #     self.mutant_before_infer = self.fuzz_node.mutant
                #     depend_fields = lib_zstack.check_field_dep(self, self._fuzz_data_logger, dependency)
                #     if not hasattr(self.fuzz_node, "last_check_depfields") and len(depend_fields) != 0:
                #         self.fuzz_node.last_check_depfields = depend_fields
                #     elif len(self.fuzz_node.last_check_depfields) > 0:
                #         depend_fields = self.fuzz_node.last_check_depfields
                #     else:
                #         self.infer_flag = False
                #
                #     if len(depend_fields) != 0:
                #         mutated = lib_zstack.mutate_with_inference(self._fuzz_data_logger, self.fuzz_node, depend_fields, self.infer_flag)
                #         self._fuzz_data_logger.log_info("Fuzzing dependent field: %s" % self.fuzz_node.mutant._name)
                #     else:
                #         mutated = self.fuzz_node.mutate()
                #     self.last_mutant = copy.copy(self.fuzz_node.render_list)
    ...
    def _fuzz_current_case(self, path):
        ...
        self._fuzz_data_logger.log_info(
            "Type: %s. Default value: %s. Current value: %s. Case %d of %d overall."
            % (
                type(self.fuzz_node.mutant).__name__,
                repr(self.fuzz_node.mutant.original_value),
                repr(self.fuzz_node.mutant._value),
