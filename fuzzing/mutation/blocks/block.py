...
class Block(IFuzzable):
    def __init__(
        ...
        self.render_list = []  # list of rendered mutant that is not None
            ...
            for item in self.stack:
                if favored is not None:
                    if isinstance(item, Block):
                        mutated = item.mutate(favored)
                        if mutated:
                            mutated = True
                            self.request.mutant = item
                            break
                    elif item._name == favored:
                        if item._fuzz_complete:
                            item.restart_mutation()
    ...
    def render(self, favored=None):
        ...
        for item in self.stack:
            if favored is None:
                data = item.render()
            elif not isinstance(item, Block) and item._name == favored.mutant_name:
                data = favored.value
            elif isinstance(item, Block):
                data = item.render(favored)
            else:
                data = item.render()
            ...
            if len(data) > 0:
                if not isinstance(item, Block):
                    ...
                    self.render_list += item.render_list
            self._rendered += data
