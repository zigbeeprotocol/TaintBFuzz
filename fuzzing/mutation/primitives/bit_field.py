...
class BitField(BasePrimitive):
    def __init__(
        ...
        self._name = name
        self._predefined_library = False
        ...
        if values and len(values) > 0:
            self._predefined_library = True
            self._fuzz_library = values
        elif self.full_range:
    ...
    def _render(self, value):
        if self._predefined_library:  # If user gives pre-defined values
            if isinstance(value, str) and self.endian == LITTLE_ENDIAN:
                temp = value[::-1]
