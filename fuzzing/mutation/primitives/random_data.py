...
class RandomData(BasePrimitive):
    def __init__(self, value, min_length, max_length, max_mutations=25, fuzzable=True, step=None, name=None, oct_string=False):
        """
    ...
    def mutate(self):
        ...
        self._value = b""
        if self.oct_string:
            str_len = random.randint(1, length)
            self._value += six.int2byte(str_len)
