...
def build_zcl_message():
    ...
    with s_block("header"):
        ...
        with s_block("manuCode", dep="frame_control", dep_values=constant.manufacturer):
            # s_word(0, endian='<', name="manu", full_range=False)
            s_static('\x01\x00', name="manu")
        s_static('\x01', name="tranSeq")
        if DEBUG:
            cmd = constant.hex_list_render([hex(0x06)])
            s_group("commandId", values=cmd)
        else:
...
def cmd_read_attribute_render():
    with s_block("command0"):
        s_word(0, endian='<', name="attributeId0", values=constant.attr_ids)
...
def cmd_read_attribute_rsp_render():
    ...
    with s_block("command1"):
        s_word(0, endian='<', name="attributeId1", values=constant.attr_ids)
        ...
        with s_block("attribute_1_1", dep="cmd_status", dep_value='\x00'):
            ...
            with s_block("attribute_value_1_1", dep="data_type1", dep_values=constant.hex_list_render([hex(0x48), hex(0x50), hex(0x51)])):
                ...
                with s_block("elements_1_1", dep="num_elements_1_1", dep_values=[0, 65535], dep_compare='!='):
                    s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_1_1", oct_string=True)
            ...
            with s_block("attribute_value_1_2", dep="data_type1", dep_value='\x4c'):
                ...
                with s_block("elements_1_2", dep="num_elements_1_2", dep_values=[0, 65535], dep_compare='!='):
                    ...
                    s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_1_2", oct_string=True)
            ...
            with s_block("attribute_value_1_3", dep="data_type1", dep_values=constant.analog_type):
                ...
                s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_1_3", oct_string=True)
...
def cmd_write_attribute_render():
    with s_block("command2"):
        s_word(0, endian='>', name="attributeId2", values=constant.attr_ids)
        ...
        with s_block("attribute_value_2_1", dep="data_type2", dep_values=constant.hex_list_render([hex(0x48), hex(0x50), hex(0x51)])):
            ...
            with s_block("elements_2_1", dep="num_elements_2_1", dep_values=[0, 65535], dep_compare='!='):
                s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_2_1", oct_string=True)
        ...
        with s_block("attribute_value_2_2", dep="data_type2", dep_value='\x4c'):
            ...
            with s_block("elements_2_2", dep="num_elements_2_2", dep_values=[0, 65535], dep_compare='!='):
                ...
                s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_2_2", oct_string=True)
        ...
        with s_block("attribute_value_2_3", dep="data_type2", dep_values=constant.analog_type):
            # cmd_data_type_value_render("data_type2", False)
            s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_2_3", oct_string=True)
...
def cmd_write_attribute_rsp_render():
    ...
    with s_block("command4"):
        if s_block_start("attribute_status"):
            ...
            s_word(0, endian='<', name="attributeId4", values=constant.attr_ids)
            s_block_end()
    ...
    with s_block("command6"):
        if s_block_start("attribute_record_6"):
            ...
            with s_block("attribute_value_6", dep="direction_6", dep_value='\x00'):
                s_word(0, endian='<', name="attributeId6", values=constant.attr_ids)
                ...
                with s_block("reportable_change_6_1", dep="data_type_6", dep_values=analog_type):
                    ...
                    s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_6", oct_string=True)
...
def cmd_config_report_rsp_render():
    ...
    with s_block("command7"):
        ...
        s_word(0, endian='<', name="attributeId7", values=constant.attr_ids)
...
def cmd_read_report_config_render():
    with s_block("command8"):
        ...
        s_word(0, endian='<', name="attributeId8", values=constant.attr_ids)
...
def cmd_read_report_config_rsp_render():
    ...
    with s_block("command9"):
        ...
        s_word(0, endian='<', name="attributeId9", values=constant.attr_ids)
        ...
        with s_block("attribute_value_9_2", dep="direction_9", dep_value='\x01'):
            ...
            with s_block("reportable_change_9_2", dep="data_type_9_2", dep_values=analog_type):
                # cmd_data_type_value_render("data_type_9", True)
                s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_9_2", oct_string=True)
...
def cmd_report_attribute_render():
    with s_block("command0a"):
        s_word(0, endian='<', name="attributeId0a", values=constant.attr_ids)
        s_group("data_type_0a", values=constant.analog_type, default_value='\x08')
        s_random("zcltest", min_length=1, max_length=64, step=1, num_mutations=64, name="data_0a", oct_string=True)
...
def cmd_discover_attribute_rsp_render():
    with s_block("command0d"):
        ...
        if s_block_start("attribute_info"):
            s_word(0, endian='<', name="attributeId0d", values=constant.attr_ids)
...
def cmd_discover_command_received_render():
    with s_block("command11"):
        s_byte(0, endian='<', name="start_cmdId", values=constant.cmd_ids)
        s_byte(1, endian='<', name="max_cmdIds", values=constant.cmd_ids)
    return
...
def cmd_discover_attribute_ext_rsp_render():
    with s_block("command16"):
        ...
        if s_block_start("attribute_ext"):
            s_word(0, endian='>', name="attributeId16", values=constant.attr_ids)
...
def main():
    session = Session(
        target=Target(connection=SocketConnection("127.0.0.1", 34567, proto='tcp', recv_timeout=60.0)),
...
def input_sequence():
    build_zcl_message()
    request = s_get("AFIncomingMSG")
    mutant = request.names["commandId"]
    mutation = 0
    inputs = []
    while mutation < mutant.num_mutations():
        if not request.block_stack:
            field_seq = {
                "data": "",
                "length": [],
                "fields": []
            ...
            for item in request.stack:
                data = item.render().decode('utf-8')
                if len(data) > 0:
                    if isinstance(item, Block):
                        for field in item.render_list:
                            field_seq["fields"].append({
                                'name': field.name,
                                'type': type(field).__name__,
                                'value': field._value
                            })
                            field_seq["length"].append(len(field))
                    else:
                        field_seq["fields"].append({
                            'name': item.name,
                        ...
                        field_seq["length"].append(len(item))
                    field_seq["data"] += data
            mutation += 1
            mutant.mutate()
            if field_seq not in inputs:
                inputs.append(field_seq)
    with open("../depenInfer/input_sequences.json", "w") as output:
        output.write(json.dumps(inputs, indent=4, ensure_ascii=False))
    
    
# -------------------------------------------------------------- #
if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == "-generate-inputs":
        input_sequence()
    elif len(sys.argv) > 1 and sys.argv[1] == "-fuzz":
        main()
    else:
        print("Please input the fuzzing options to be performed.\n"
              "\t-generate-inputs\t\tgenerate input representative sequences\n"
              "\t-fuzz\t perform fuzzing test based on the pre-defined format script")
