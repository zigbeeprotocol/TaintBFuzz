...
let add_code_annot ?(keep_empty=true) emitter ?kf stmt ca =
  ...
  | AAssert(l, p) ->
    ...
    (* TODO: May need enable later *)
    (*let looking_for x = String.equal x "mem_access" in
    if List.exists looking_for p.tp_statement.pred_name then begin
	Kernel.feedback "Find mem_access assertion.";
	remove_code_annot_internal emitter ~remove_statuses:false ~kf stmt a;
	fill_tables a []
    end else begin
