...
let emit_status emitter kf stmt annot status =
  ...
  Kernel.debug "property=%a" Property.pretty p;
...
let add_annotation tbl alarm emitter ?kf kinstr annot =
  Kernel.debug "Add annotation %a" D.pretty alarm;
...
let create_predicate ?(loc=Location.unknown) alarm =
  let aux = function
    ...
    | Memory_access(lv, read) -> 
      ...
      let t = Logic_utils.expr_to_term e in
      (* Address ZCL mem_access valid *)
      let filename = Filepath.Normalized.to_pretty_string (fst t.term_loc).pos_path in
      let contains s1 s2 =
      ...
      in
        if contains filename "zcl.c" then
          Kernel.feedback "Alarms: Find loc=%s" filename;
...
let register emitter ?kf kinstr ?loc ?status alarm =
  ...
  if fresh then begin
    ...
    in
    match alarm with
      (* Temporarily remove mem_access annot for zcl. *)
      | Memory_access(_,_) ->
        let kf, stmt = alarm_kf_stmt ?kf kinstr in
        Kernel.debug "Remove code annot mem_access";
		Annotations.remove_code_annot emitter ~kf stmt annot
      | _ ->
  ...
  | Some status ->
    ...
    (* Temporarily remove mem_access annot for zcl. *)
	  | Memory_access(_,_) -> ()
	  | _ -> emit_status emitter kf stmt annot status;
  | _ -> ()
  end;
  annot, fresh
