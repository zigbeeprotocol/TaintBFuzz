...
def parse_zcl_cfg(module, cfg_file):
    print("Starting to parse the CFG file...");
    cfg = open(cfg_file, "r")
    func_new = False
    block_new = False
    module.name = 'zcl'
    ...
    for line in cfg:
        ...
        elif current_function is not None and current_block is not None and int(current_block.block_number) == current_function.total_blocks and '}' in line:
            func_new = False
            block_new = False
    zcl_module.total_statements = zcl_module.num_statements()
    zcl_module.total_edges = zcl_module.num_edges()
    cfg.close()
    return
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        if zcl_module.total_functions > 0:
            ...
            cfg.close()
            print("Save parsing result as a JSON file: " + os.path.abspath(output))
    else:
